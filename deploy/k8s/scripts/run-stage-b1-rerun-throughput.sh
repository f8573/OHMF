...
RESULT_DIR="${REPO_ROOT}/benchmarks/results/${RUN_DATE}-stage-b1-rerun-throughput"
DEPLOY_RESULT_MD="deploy/k8s/results/${RUN_DATE}-stage-b1-rerun-throughput.md"
...
IMAGE_TAG_SUFFIX="${IMAGE_TAG_SUFFIX:-b1rerun-${RUN_STAMP}}"
...
shard_counts_for_rate() {
  case "$1" in
    ...
    90) echo "8,8,8,8,8,8,7,7,7,7,7,7" ;;
...
per_pod_label_for_rate() {
  case "$1" in
    ...
    90) echo "mixed 7-8 msg/sec per pod" ;;
...
for rate in "${rates[@]}"; do
  ...
  scenario="stage-b1-rerun-${rate}msgsec-multisource"
  result_dir="${RESULT_DIR}/${rate}msgsec"
  ...
  echo "=== Stage B1 rerun rung ${rate} msg/sec ==="
  SCENARIO_NAME="${scenario}" \
  RESULT_DIR="${result_dir}" \
  DEPLOY_RESULT_MD="${deploy_result_md}" \
  DEPLOY_TITLE="Local k3s Stage B1 rerun rung - ${rate} msg/sec multisource - ${RUN_DATE}" \
...
python "${REPO_ROOT}/benchmarks/scripts/summarize_stage_b1_rerun_throughput.py" \
