...
NS="${NS:-ohmf}"
SCENARIO_NAME="${SCENARIO_NAME:-processor-backlog-recovery-120msgsec}"
...
DEPLOY_TITLE="${DEPLOY_TITLE:-Local k3s processor backlog recovery result - 120 msg/sec - ${RUN_DATE}}"
BENCHMARK_LABEL="${BENCHMARK_LABEL:-${SCENARIO_NAME}}"
JOB_NAME="${JOB_NAME:-loadgen-${SCENARIO_NAME}}"
RUN_ID_PREFIX="${RUN_ID_PREFIX:-${RUN_STAMP}-backlog}"
...
SCALE_UP_OFFSET_SECONDS="${SCALE_UP_OFFSET_SECONDS:-300}"
...
gateway_image="ohmf-gateway:backlog-${RUN_STAMP}"
apps_image="ohmf-apps:backlog-${RUN_STAMP}"
processor_image="ohmf-messages-processor:backlog-${RUN_STAMP}"

OBS_DIR="${RESULT_DIR}/observations"
SHARDS_DIR="${RESULT_DIR}/shards"
MANIFEST_JSON="${OBS_DIR}/run-manifest.json"
SCALE_DOWN_EVENT_JSON="${OBS_DIR}/scale-down-event.json"
SCALE_UP_EVENT_JSON="${OBS_DIR}/scale-up-event.json"
...
scale_processor_to_zero() {
  log "Scaling messages-processor to 0 replicas (backlog-build phase)"
  local scale_down_at_utc
  scale_down_at_utc="$(date -u +%FT%TZ)"
  kubectl -n "${NS}" scale deploy/messages-processor --replicas=0

  local started_at now count
  started_at="$(date +%s)"
  while true; do
    count="$(kubectl -n "${NS}" get pods -l app.kubernetes.io/name=messages-processor --no-headers 2>/dev/null | grep -c Running || true)"
    if [[ "${count}" -eq 0 ]]; then
      break
    fi
    now="$(date +%s)"
    if (( now - started_at >= 120 )); then
      fail "messages-processor pods did not scale down within 120s"
    fi
    sleep 5
  done

  cat > "${SCALE_DOWN_EVENT_JSON}" <<EOF
{
  "scale_down_at_utc": "${scale_down_at_utc}",
  "replicas_before": 4,
  "replicas_after": 0
}
EOF
  log "messages-processor scaled to 0; backlog will accumulate in Kafka"
}
  ...
  cat > "${MANIFEST_JSON}" <<EOF
{
  ...
  "scale_up_offset_seconds": ${SCALE_UP_OFFSET_SECONDS},
...
cleanup() {
  ...
  if [[ -n "${SCALE_UP_PID}" ]] && kill -0 "${SCALE_UP_PID}" 2>/dev/null; then
    kill "${SCALE_UP_PID}" 2>/dev/null || true
    wait "${SCALE_UP_PID}" 2>/dev/null || true
  fi
}
trap cleanup EXIT
      ...
      containers:
        - name: loadgen
          ...
          env:
            ...
            - name: MESSAGE_TEXT
              value: "backlog recovery 120msgsec local k3s validation"
EOF
}
...
scale_up_processors_mid_run() {
  (
    sleep "${SCALE_UP_OFFSET_SECONDS}"

    local pre_scaleup_stamp="pre-scaleup"
    capture_pods_state "${pre_scaleup_stamp}"
    capture_kafka_state "${pre_scaleup_stamp}"
    capture_gateway_logs "${pre_scaleup_stamp}"
    capture_cluster_events "${pre_scaleup_stamp}"
    capture_processor_metrics "${pre_scaleup_stamp}"
    capture_processor_logs "${pre_scaleup_stamp}"

    local scale_up_at_utc scale_up_at_epoch
    scale_up_at_utc="$(date -u +%FT%TZ)"
    scale_up_at_epoch="$(python - <<'PY'
import time
print(time.time())
PY
)"

    cat > "${SCALE_UP_EVENT_JSON}" <<EOF
{
  "scale_up_at_utc": "${scale_up_at_utc}",
  "scale_up_at_epoch": ${scale_up_at_epoch},
  "scale_up_offset_seconds": ${SCALE_UP_OFFSET_SECONDS},
  "replicas_before": 0,
  "replicas_after": 4
}
EOF
    ...
    log "Scaling messages-processor back to 4 at ${scale_up_at_utc}"
    kubectl -n "${NS}" scale deploy/messages-processor --replicas=4
    kubectl -n "${NS}" rollout status deploy/messages-processor --timeout=300s

    capture_pods_state "post-scaleup"
    capture_kafka_state "post-scaleup"
    capture_gateway_logs "post-scaleup"
    capture_cluster_events "post-scaleup"
    capture_processor_metrics "post-scaleup"
    capture_processor_logs "post-scaleup"

    wait_for_consumer_group_assignment 4 900 10 || true

    capture_pods_state "during-recovery"
    capture_kafka_state "during-recovery"
    capture_gateway_logs "during-recovery"
    capture_cluster_events "during-recovery"
    capture_processor_metrics "during-recovery"
    capture_processor_logs "during-recovery"
  ) &
  SCALE_UP_PID=$!
}
...
main() {
  ...
  scale_processor_to_zero
  capture_processor_rollup "scale-down"
  ...
  log "Aggregating backlog recovery results"
  python "${REPO_ROOT}/benchmarks/scripts/summarize_processor_backlog_recovery.py" \
    ...
    --scale-up-offset-seconds "${SCALE_UP_OFFSET_SECONDS}" \
    --run-status "${RUN_STATUS}"

  ok "Processor backlog recovery artifacts written to ${RESULT_DIR}"
}
